 ...
 * Add your docs here.
 */
public class SwitchHandler extends ConstantHandlerEG implements Exceptions {
    ...
    {
        return new Switch(name, ConstantHandlerEG.addConstantBool(name, true));
    }
    
    /**
     * Adds a boolean switch to the shuffleboard's Prefrences instance by input key and 
     * with a init value by input, and returns the supplier for that switch.
     * @param name - The key for the shuffleboard switch
    ...
    {
        return new Switch(name, ConstantHandlerEG.addConstantBool(name, defaultValue));
    }
