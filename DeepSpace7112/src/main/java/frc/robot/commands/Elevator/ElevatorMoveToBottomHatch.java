...
public class ElevatorMoveToBottomHatch extends CommandGroup {
  /**
   * The command makes the elevator go to the bottom hatch using a PID loop with the encoder.
   * If the elevator passes by the hall effect sensor the encoder will reset.
   */
  public ElevatorMoveToBottomHatch() {
    ...
    addSequential(new MoveBasicSubsystemWithPID(Robot.elevator, SubsystemComponents.Elevator.encoder, 
                  ...
                  SubsystemConstants.Elevator.kWaitTime.get()), 2,false));
